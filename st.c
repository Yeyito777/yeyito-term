...
strhandle(void)
{
	...
	case ']': /* OSC -- Operating System Command */
		...
		case 112: /* reset dynamic text cursor color */
			...
			return;
		case 777: /* st custom: vim-mode and cursor notifications */
			if (narg >= 3 && !strcmp(strescseq.args[1], "vim-mode")) {
				if (!strcmp(strescseq.args[2], "enter"))
					vimnav_enter();
				else if (!strcmp(strescseq.args[2], "exit"))
					vimnav_exit();
			} else if (narg >= 3 && !strcmp(strescseq.args[1], "cursor")) {
				/* zsh reports cursor position: \033]777;cursor;POS\a */
				int pos = atoi(strescseq.args[2]);
				vimnav_set_zsh_cursor(pos);
			} else if (narg >= 3 && !strcmp(strescseq.args[1], "visual")) {
				/* zsh reports visual mode state */
				if (!strcmp(strescseq.args[2], "end")) {
					vimnav_set_zsh_visual(0, 0, 0);
				} else if (narg >= 5 && !strcmp(strescseq.args[2], "start")) {
					/* \033]777;visual;start;ANCHOR;TYPE\a (TYPE: char or line) */
					int anchor = atoi(strescseq.args[3]);
